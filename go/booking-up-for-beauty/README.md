...
A [`Time`][time] in Go is a type describing a moment in time.
...
The current date and time can be retrieved through the [`time.Now`][now] function.

The [`time.Parse`][parse] function parses strings into values of type `Time`.
...
The [`Time.Format()`][format] method returns a string representation of time.
...
In Go predefined date and timestamp [format constants][const] are also available.
