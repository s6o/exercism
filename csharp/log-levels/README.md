...
A `string` in C# is an object that represents immutable text as a sequence of
Unicode characters (letters, digits, punctuation, etc.). Double quotes are used
to define a `string` instance:
...
For example, to remove the whitespace from a string, you can use the
[Trim method][tutorial-docs.microsoft.com-trim-white-space] like this:
...
Multiple strings can be concatenated (added) together. The simplest way to
achieve this is by using the `+` operator.
...
For any string formatting more complex than simple concatenation, string
interpolation is preferred. To use interpolation in a string, prefix it with the
dollar (`$`) symbol. Then you can use curly braces (`{}`) to access any variables
inside your string.
...
You have three tasks, each of which will take a log line and ask you to do
something with it.
...
Implement the (_static_) `LogLine.LogLevel()` method to return a log line's log
...
Implement the (_static_) `LogLine.Reformat()` method that reformats the log line,
