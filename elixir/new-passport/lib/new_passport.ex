defmodule NewPassport do
  def get_new_passport(now, birthday, form) do
    with {:ok, building_ts} <- enter_building(now),
         {:ok, counter_fn} <- find_counter_information(now),
         counter <- counter_fn.(birthday),
         {:ok, checksum} <- stamp_form(building_ts, counter, form),
         passport_number <- get_new_passport_number(building_ts, counter, checksum) do
      {:ok, passport_number}
    else
      {:coffee_break, _msg} ->
