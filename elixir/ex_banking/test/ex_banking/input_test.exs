defmodule ExBanking.InputTest do
  use ExUnit.Case

  test "only with user" do
    {:ok, data} = ExBanking.Input.create("john")
  ...
  test "with user and currency" do
    {:ok, data} = ExBanking.Input.create("john", "EUR")
    assert is_nil(data.amount)
  ...
  test "with user, currency and amount" do
    {:ok, data} = ExBanking.Input.create("john", "EUR", 1.23)
  ...
  test "empty user string" do
    assert ExBanking.Input.create("") == {:error, :wrong_arguments}
  end
  ...
  test "with user, but empty currency string" do
    assert ExBanking.Input.create("john", "") == {:error, :wrong_arguments}
  end

  test "with user and currency, but 0 amount" do
    assert ExBanking.Input.create("john", "EUR", 0) == {:error, :wrong_arguments}
  end

  test "with user, currency, but negative amount" do
    assert ExBanking.Input.create("john", "EUR", -24) == {:error, :wrong_arguments}
  end

  test "with user, currency, but negative float amount" do
    assert ExBanking.Input.create("john", "EUR", -2.42) == {:error, :wrong_arguments}
  end
end
