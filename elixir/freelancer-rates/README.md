...
Floats are numbers with one or more digits behind the decimal separator. They
...
In the [`Integer`][integer-functions] and [`Float`][float-functions] modules you
can find some useful functions for working with those types. Basic arithmetic
operators are defined in the [`Kernel`][kernel-arithmetic-operators] module.

### Conversion

Integers and floats can be mixed together in a single arithmetic expression.
Using a float in an expression ensures the result will be a float too.
...
However, when doing division, the result will always be a float, even if only
integers are used.
...
To convert a float to an integer, you can discard the decimal part with
[`trunc/1`][trunc-1].
...
project manager by providing a few utilities to quickly calculate daily and
...
Discounts are modeled as fractional numbers representing percentage, for example
`25.0` (25%).
...
The returned monthly rate should be rounded up (take the ceiling) to the nearest
nteger.

## 4. Calculate the number of workdays given a budget, hourly rate and discount

Implement a function that takes a budget, a hourly rate, and a discount, and
